...
PLACEHOLDER_BLOCKLIST = [
    ...
    "1e2c060c-e40b-4f3f-9b94-9febcbe4e04f",
]

# Env vars that satisfy each placeholder (set via GitHub Variables in verity-guardrails.yml)
PLACEHOLDER_ENV_OVERRIDES = {
    "https://localhost:3001/api/callbacks/github-actions": "VERITY_CALLBACK_URL",
    "1e2c060c-e40b-4f3f-9b94-9febcbe4e04f": "VERITY_PROJECT_ID",
}
    ...
    if config_path.exists() and not template_source_repo:
        ...
        for placeholder in PLACEHOLDER_BLOCKLIST:
            if placeholder in config_text:
                env_var = PLACEHOLDER_ENV_OVERRIDES.get(placeholder, "")
                if env_var and os.environ.get(env_var, "").strip():
                    continue  # value supplied via GitHub Variable — skip
